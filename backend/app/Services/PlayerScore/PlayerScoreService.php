...
class PlayerScoreService
{
    ...
    {
        if ($existing) {
            $data = array_merge($existing->only($existing->getFillable()), $data);
        }
        ...
        if (($data['status'] ?? null) === PlayerScoreStatus::DidNotPlay->value
        ...
        $matchdayId = $data['matchday_id'] ?? null;
        ...
        $rules = [
            ...
            'matchday_id' => ['required', 'integer', Rule::exists('matchdays', 'id')],
            'base_rating' => ['nullable', 'numeric', 'decimal:0,2', 'between:-99.99,99.99'],
            'final_score' => [
                Rule::requiredIf(($data['status'] ?? null) === PlayerScoreStatus::Confirmed->value
