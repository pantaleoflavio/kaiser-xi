...
/**
 * Validates and persists formation structure without applying request-time lifecycle rules.
 *
 * Production writes must enter through SaveFormationService. This lower-level writer exists
 * for trusted state construction such as historical demo fixtures.
 */
final class FormationPersistenceService
{
    /** @param array{formation_module_id: int, starters: list<int>, bench: list<array{fantasy_team_player_id: int, order: int}>} $data */
    public function persist(League $league, Matchday $matchday, FantasyTeam $team, array $data): Formation
    {
