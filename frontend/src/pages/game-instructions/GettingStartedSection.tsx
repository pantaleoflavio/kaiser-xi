...
export function GettingStartedSection() {
  ...
  const details = tdList('chapters.gettingStarted.details');
  ...
  return (
    ...
    >
      <p className="text-sm font-semibold text-theme-accent">{td('chapter', { number: 1 })}</p>

      <h2 className="mt-1 text-2xl font-bold text-theme-text">
        {td('chapters.gettingStarted.title')}
      </h2>

      <p className="mt-4 leading-7 text-theme-muted">{td('chapters.gettingStarted.overview')}</p>
      ...
      <aside className="mt-6 rounded-xl border border-theme-accent/50 bg-theme-muted-surface p-4">
        <strong className="text-theme-text">{td('chapters.gettingStarted.calloutLabel')}</strong>

        <p className="mt-1 text-sm leading-6 text-theme-muted">
          {td('chapters.gettingStarted.callout')}
        </p>
      </aside>

      <nav
        aria-label={td('chapterNavigation')}
        className="mt-8 flex justify-end border-t border-theme-border pt-5 text-sm font-semibold"
      >
        <a className="text-theme-accent hover:underline" href="#creating-a-league">
          {t('common.next')}: {td('chapters.creatingLeague.title')} →
        </a>
      </nav>
    </article>
  );
