...
export function LeagueInitialRulesForm({ league, settings, isSaving, serverErrors, saveFailed, onSave }: Props) {
  ...
  const [mutability, setMutability] = useState({
    budget_rules_mutable: settings.budget_rules_mutable,
    roster_size_mutable: settings.roster_size_mutable,
    roster_role_limits_mutable: settings.roster_role_limits_mutable,
  });
  const [errors, setErrors] = useState<Record<string, string>>({});
  useEffect(() => { setBudget(String(settings.initial_budget ?? '')); setRefund(String(settings.release_refund_percentage ?? '')); setMaximum(String(settings.max_roster_players)); setLimits(Object.fromEntries(roles.map(role => [role, String(settings.roster_role_limits[role])])) as Record<PlayerRoleKey, string>); setMutability({ budget_rules_mutable: settings.budget_rules_mutable, roster_size_mutable: settings.roster_size_mutable, roster_role_limits_mutable: settings.roster_role_limits_mutable }); }, [settings]);
  ...
  function submit(event: SubmitEvent<HTMLFormElement>) {
    ...
    if (Object.values(mutability).some(value => typeof value !== 'boolean')) next.mutability = t('leagueCreate.validation.mutability');
    if (Object.keys(next).length) { setErrors(next); return; }
    onSave({ initial_budget: parsedBudget, release_refund_percentage: parsedRefund, max_roster_players: parsedMaximum, roster_role_limits: parsedLimits, ...mutability });
  }
  ...
  return <form className="grid gap-5" noValidate onSubmit={submit}>
    ...
    <fieldset className="grid gap-4 sm:grid-cols-2"><legend className="mb-2 text-base font-semibold text-white">{t('leagueCreate.fields.roleLimits')}</legend>{roles.map(role => <label className="text-sm text-slate-200" key={role}>{t(`leagueCreate.roles.${role}`)}<input className={inputClass} disabled={isSaving} min="0" step="1" type="number" value={limits[role]} onChange={e => setLimits({...limits, [role]: e.target.value})}/>{error(`roster_role_limits.${role}`)}</label>)}<div className="sm:col-span-2">{error('roster_role_limits')}<p className="text-sm text-slate-400">{t('leagueCreate.fields.roleLimitsHelp')}</p></div></fieldset>
        <fieldset className="grid gap-3"><legend className="text-base font-semibold text-white">{t('leagueCreate.flexibility.title')}</legend><p className="text-sm text-slate-400">{t('leagueCreate.flexibility.help')}</p>{([
      ['budget_rules_mutable', 'budget'],
      ['roster_size_mutable', 'rosterSize'],
      ['roster_role_limits_mutable', 'roleLimits'],
    ] as const).map(([field, label]) => <label className="flex items-start gap-3 rounded-lg border border-slate-700 p-3 text-sm text-slate-200" key={field}><input checked={mutability[field]} className="mt-1 size-4" disabled={isSaving} onChange={event => setMutability(current => ({ ...current, [field]: event.target.checked }))} type="checkbox"/><span><span className="font-semibold">{t(`leagueCreate.flexibility.${label}`)}</span><span className="mt-1 block text-slate-400">{t(mutability[field] ? 'leagueCreate.flexibility.editable' : 'leagueCreate.flexibility.locked')}</span></span></label>)}{error('mutability')}</fieldset>
