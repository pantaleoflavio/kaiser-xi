...
export function HeadToHeadScheduleSummary({ schedule }: { schedule: HeadToHeadSchedule }) {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-emerald-400/30 bg-emerald-950/20 p-6">
        <h1 className="text-3xl font-bold text-white">{t('h2h.title')}</h1>
        <p className="mt-2 font-semibold text-emerald-200" role="status">
          {t('h2h.generated')}
        </p>
        <dl className="mt-5 grid gap-4 sm:grid-cols-3">
          <div>
            <dt className="text-sm text-slate-400">{t('h2h.generatedAt')}</dt>
            <dd className="mt-1 text-white">
              {formatDate(schedule.generated_at, t('leagueDetail.notAvailable'), language)}
            </dd>
          </div>
          <div>
            <dt className="text-sm text-slate-400">{t('h2h.startingMatchday')}</dt>
            <dd className="mt-1 text-white">{label}</dd>
          </div>
          <div>
            <dt className="text-sm text-slate-400">{t('h2h.currentParticipants')}</dt>
            <dd className="mt-1 text-white">{schedule.participant_count}</dd>
          </div>
        </dl>
      </section>
      <section aria-labelledby="fixtures-heading">
        <h2 className="text-2xl font-semibold text-white" id="fixtures-heading">
          {t('h2h.schedule')}
        </h2>
            ...
            >
              ...
              <ul className="mt-4 space-y-2">
                {group.fixtures.map((fixture) => (
                  <li
                    className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 rounded-lg bg-slate-950/60 px-3 py-3 text-sm text-white"
                    key={fixture.id}
                  >
                    <span className="min-w-0 break-words text-right">
                      {fixture.home_fantasy_team.name}
                    </span>
                    <span className="text-slate-400">{t('h2h.versus')}</span>
